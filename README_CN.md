...
不要把真实域名、IP、token、client secret 或个人知识库路径提交到 GitHub。真实值应放在
`.env`、部署平台 secret，或本机私有配置中。
